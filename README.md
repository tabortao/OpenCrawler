...
- 🤖 **MCP Server** - 支持 Model Context Protocol，可与 AI 工具集成
...
# macOS / Linux（bash/zsh）用 \ 续行；Windows PowerShell 用反引号 ` 续行。
...
# 编辑 .env 文件，配置 API_TOKEN 和 Cookie
# vim .env
...
| 容器路径 | 说明 |
|----------|------|
| `/app/output` | 爬取的 Markdown 文件输出目录 |
| `/app/data` | 配置文件和数据目录 |
| `/app/browser_data` | 浏览器数据（Cookie、缓存） |
...
OpenCrawler 内置 MCP (Model Context Protocol) Server，可与 AI 工具（如 Trae、Cursor、Cherry Studio）集成。
...
# 或只启动 MCP 服务
docker compose up opencrawler-mcp -d
```
...
```bash
# 运行 MCP HTTP 服务
docker run -d --name opencrawler-mcp \
  -p 8765:8765 \
  -v $(pwd)/output:/app/output \
  -v $(pwd)/data:/app/data \
  ghcr.io/tabortao/opencrawler:latest \
  python mcp_server.py http --host 0.0.0.0 --port 8765
```
